...
class DocumentGeneratorService {
  ...
  static parseContentToTableData(content) {
    ...
    // Helper to identify separator line
    ...
    lines.forEach(line => {
      ...
      if (isTableSeparator(trimmed)) {
        return; // Skip separator lines like |---|---|
      }

      if (line.includes('|')) {
        // Pipe-delimited table
        let parts = line.split('|').map(cell => cell.trim());
        // If it starts and ends with '|', remove the outer empty cells caused by splitting
  ...
  static createWordDocument(title, sections) {
    ...
    paragraphs.push(
      new Paragraph({
        children: [new TextRun({ text: `Generated: ${new Date().toLocaleString()}`, italics: true, size: 20, color: '666666' })],
        spacing: { after: 200 }
      })
    );
    ...
    sections.forEach(section => {
      if (section.type === 'heading') {
        paragraphs.push(
          new Paragraph({
            children: [new TextRun({ text: section.text, bold: true, size: 26 })],
