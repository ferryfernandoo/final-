...
class ImageGenerationService {
  ...
  static async generateImage(prompt, size = '1024x1024', sessionId = null, model = null, referenceImage = null, abortSignal = null) {
    try {
      ...
      const directKey = TOKENMIX_API_KEY;
      const canUseDirectTokenMix = !!directKey;

      if (canUseDirectTokenMix) {
        console.log('🔴🔴🔴 [IMAGE_GEN_DEBUG] Local TokenMix API key detected. Trying direct TokenMix API first.');
        try {
          ...
          if (isEditMode) {
            ...
            if (refImageUrl.startsWith('data:')) {
              ...
              // Attempt raw base64 as fallback
        ...
        console.warn('🔴🔴🔴 [IMAGE_GEN_DEBUG] VITE_TOKENMIX_API_KEY missing, falling back to backend proxy.');
      }
