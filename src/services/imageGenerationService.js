...
class ImageGenerationService {
  ...
  static async generateImage(prompt, size = '1024x1024', sessionId = null, model = null, referenceImage = null, abortSignal = null) {
    try {
      ...
      // Auto-select correct model: image-01 for generation, qwen-image-edit-max for edit
      let finalModel = model;
      if (!finalModel || finalModel === 'qwen-vl-max') {
        finalModel = isEditMode ? 'qwen-image-edit-max' : 'image-01';
      }
      if (isEditMode && (finalModel === 'image-01' || finalModel === 'imagen-4-fast')) {
