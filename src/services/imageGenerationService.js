...
class ImageGenerationService {
  ...
  static async generateImage(prompt, size = '1024x1024', sessionId = null, model = null, referenceImage = null, abortSignal = null) {
    try {
      ...
      // Normalize referenceImage parameter to a valid URL or dataUrl string
      let refImageUrl = null;
      if (referenceImage) {
        if (typeof referenceImage === 'string') {
          refImageUrl = referenceImage;
        } else if (typeof referenceImage === 'object') {
          refImageUrl = referenceImage.dataUrl || referenceImage.url || referenceImage.base64 || null;
        }
        if (refImageUrl && typeof refImageUrl === 'string' && !refImageUrl.startsWith('data:') && !refImageUrl.startsWith('http')) {
          refImageUrl = `data:image/jpeg;base64,${refImageUrl}`;
        }
      }

      const isEditMode = !!refImageUrl;
      const modeLabel = isEditMode ? '✏️ EDIT' : '🎨 GENERATE';
      
      // Auto-select correct model: qwen-image for generation, qwen-image-edit-max for edit
      let finalModel = model;
      if (!finalModel || finalModel === 'qwen-vl-max' || finalModel === 'image-01' || finalModel === 'imagen-4-fast') {
        finalModel = isEditMode ? 'qwen-image-edit-max' : 'qwen-image';
      }
      if (isEditMode && (finalModel === 'qwen-image' || finalModel === 'image-01' || finalModel === 'imagen-4-fast')) {
        finalModel = 'qwen-image-edit-max';
      }
      ...
      const fetchOptions = {
        ...
        body: JSON.stringify({
          ...
          model: finalModel,
          sessionId,
          ...(refImageUrl && { referenceImage: refImageUrl }), // Only include if provided
        }),
      };
      ...
      const directKey = TOKENMIX_API_KEY || (typeof import.meta !== 'undefined' && import.meta.env?.VITE_TOKENMIX_API_KEY) || 'sk-tm-7rvNarobwrId5fwOYprZl0LptIHGcaJ4GtpAheTJlhbt3kDC';
      ...
      if (!response.ok) {
        ...
        let errorMsg = 'Model gambar sedang sangat sibuk, coba lagi nanti.';
        try {
          const parsed = JSON.parse(errorText);
          if (parsed.error && typeof parsed.error === 'string' && parsed.error.includes('melanggar kebijakan')) {
      ...
      try {
        ...
        console.error('🔴🔴🔴 [IMAGE_GEN_DEBUG] Failed to parse JSON response:', parseErr);
  ...
  static detectImageRequest(userMessage) {
    ...
    // Indonesian patterns - with verb variations (buat, buatkan, buatnya, gambarin, lukisan, edit, ubah, etc)
    const indonesianPatterns = [
      // Edit patterns: "edit gambar/foto/ini ...", "ubah [gambar/foto/ini] ...", "ganti [latar/baju/warna] ...", "jadikan [anime/cyberpunk] ..."
      /(?:edit|ubah|ganti|modifikasi|perbaiki)\s+(?:gambar|foto|image|ini|itu)?\s*(.+?)(?:\s+ukuran\s+|$)/i,
      /(?:bikin|jadikan|ubah)\s+(?:foto|gambar|ini)\s+(?:jadi|seperti|bergaya)\s+(.+?)(?:\s+ukuran\s+|$)/i,
  ...
  static async generateEnglishImagePrompt(originalPrompt) {
    ...
    try {
      ...
      if (isGuestMode) {
        ...
        const apiKey = (typeof import.meta !== 'undefined' && import.meta.env?.VITE_DEEPSEEK_API_KEY) || (typeof import.meta !== 'undefined' && import.meta.env?.VITE_TOKENMIX_API_KEY) || 'sk-62106eda06b7406f8cd13b9849cd19e5';
        if (apiKey) {
...
Format your response in a professional but accessible way. Make it sound like you're explaining your thinking process step-by-step.`;
   ...
   */
  static detectImageEditRequest(userMessage, hasReferenceImage = false) {
    if (hasReferenceImage) {
      // If user explicitly asks for a new image from scratch without using the reference image
      const isExplicitNewFromScratch = /from scratch|gambar baru (?:terpisah|tanpa acuan|dari nol)|new image from scratch/i.test(userMessage || '');
      if (isExplicitNewFromScratch) {
        console.log('[IMAGE_EDIT_DETECT] Explicit new image from scratch requested despite reference image');
        return false;
      }
      // Reasoning-based Edit Mode: When a reference image is present, treat image requests as Edit/Image-to-Image transformations by default
      console.log('[IMAGE_EDIT_DETECT] ✅ Reference image present in context — using Edit Mode based on reasoning');
      return true;
    }
    ...
    for (const pattern of allPatterns) {
      if (pattern.test(lowerMessage)) {
        console.log('[IMAGE_EDIT_DETECT] ✅ Image edit request detected via pattern');
        return true;
      }
    }
