...
test('getTokenMixModel uses stable Grok for both text and image payloads', async () => {
  const { getTokenMixModel } = await import('./grokApi.js');
  assert.equal(getTokenMixModel('deepernova-1.2-flash', false), 'grok-4.1-fast-reasoning');
  assert.equal(getTokenMixModel('deepernova-2.3-pro', true), 'grok-4.1-fast-reasoning');
});
...
test('getTokenMixModel routes spaced Deepernova aliases to stable Grok for image uploads', () => {
  assert.equal(getTokenMixModel('deepernova 2.4 pro', true), 'grok-4.1-fast-reasoning');
  assert.equal(getTokenMixModel('deepernova 2.4 pro', false), 'grok-4.1-fast-reasoning');
});
