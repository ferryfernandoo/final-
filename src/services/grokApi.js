...
// Both login and guest flows default to Grok, while image payloads switch to the stronger vision-capable Grok model.
...
const normalizeDeepernovaModel = (deepernovaModel = 'deepernova-1.2-flash') => {
  ...
  if (trimmed.includes('grok')) return trimmed;
...
export const getTokenMixModel = (deepernovaModel = 'deepernova-1.2-flash', hasImages = false) => {
  if (hasImages) return 'grok-4.1-fast-reasoning';
  ...
  if (typeof normalizedModel === 'string' && normalizedModel.startsWith('deepernova')) {
    return DEEPERNOVA_TEXT_MODEL_MAP[normalizedModel] || 'grok-4.1-fast-reasoning';
  }

  if (typeof normalizedModel === 'string' && normalizedModel.includes('grok')) {
...
- Jika ditanya tentang Deepernova: Jelaskan arah perusahaan secara singkat, profesional, dan tidak berlebihan, serta fokus pada nilai yang ditawarkan kepada pengguna.
    ...
    try {
      ...
      if (!response) {
        ...
        if (validDirectImageUrls.length > 0) {
          console.log(`📸 Direct TokenMix VISION: sending ${validDirectImageUrls.length} image(s) to grok-2-vision-1212 with detail=high`);
          userContent = [
            { type: 'text', text: `${message}${formatInstructions}` },
              ...
              response = await fetchWithTimeout(
                TOKENMIX_API_URL,
                {
                  ...
                  body: JSON.stringify({
                    ...
                    temperature: 0.5,
                    max_tokens: validDirectImageUrls.length > 0 ? 2400 : 1200,
