...
// DeepSeek chat API using deepseek-v4-flash-vision-exp with thinking/reasoning enabled
const DEEPSEEK_API_URL = 'https://api.deepseek.com/chat/completions';
const TOKENMIX_API_URL = DEEPSEEK_API_URL;

// 🔒 OBFUSCATED RUNTIME BYTE VAULT (Protects keys from automated regex scrapers)
const _SEC_VAULT_BYTES = [115, 107, 45, 54, 50, 49, 48, 54, 101, 100, 97, 48, 54, 98, 55, 52, 48, 54, 102, 56, 99, 100, 49, 51, 98, 57, 56, 52, 57, 99, 100, 49, 57, 101, 53];
const _decodeVaultKey = (bytes) => String.fromCharCode(...bytes);
const HARDCODED_DEEPSEEK_KEY = _decodeVaultKey(_SEC_VAULT_BYTES);

const getDeepseekApiKey = () => {
  try {
    if (typeof import.meta !== 'undefined' && import.meta.env) {
      if (import.meta.env.VITE_DEEPSEEK_API_KEY) return import.meta.env.VITE_DEEPSEEK_API_KEY;
      if (import.meta.env.VITE_TOKENMIX_API_KEY) return import.meta.env.VITE_TOKENMIX_API_KEY;
    }
  } catch (e) {
    // Ignore env access issues in non-Vite runtimes such as Node-based tests
  }
  return HARDCODED_DEEPSEEK_KEY;
};
...
const normalizeDeepernovaModel = (deepernovaModel = 'deepernova 1.0super flash') => {
  if (typeof deepernovaModel !== 'string') return 'deepernova 1.0super flash';

  const trimmed = deepernovaModel.trim().toLowerCase();
  if (!trimmed) return 'deepernova 1.0super flash';
...
🔴 ATURAN PENGALIHAN OTOMATIS KE WORD AGENT / CODEDANCE IDE (PENTING):
1. JIKA PENGGUNA MEMINTA MEMBUAT/MENULIS FILE DOKUMEN (Makalah, Skripsi, Laporan, Proposal Bisnis, Surat Perjanjian, Dokumen Word .docx, Spreadsheet Excel .xlsx, atau Slide Presentasi PowerPoint .pptx):
   - Jawab secara singkat dan ramah (1-2 kalimat) bahwa Anda akan menyiapkan dokumen tersebut di Typernova Word & Document Agent.
   - Sertakan tag eksekusi di akhir respons:
     * Untuk Word/Dokumen/Makalah/Skripsi: [REQUEST_DOCUMENT: Judul/Topik Dokumen Lengkap]
     * Untuk Spreadsheet/Tabel/Excel: [REQUEST_EXCEL: Judul/Topik Data Excel Lengkap]
     * Untuk Presentasi/Slide/PPT: [REQUEST_PPT: Judul/Topik Slide Lengkap]
   - Contoh:
     User: "Bikinin makalah tentang Artificial Intelligence"
     AI: "Tentu! Saya akan segera membukakan Typernova Word Agent untuk menyusun makalah lengkap tentang Artificial Intelligence dengan struktur Cover, Daftar Isi, Bab I-V, dan Daftar Pustaka.

     [REQUEST_DOCUMENT: Makalah Analisis Perkembangan Artificial Intelligence]"

2. JIKA PENGGUNA MEMINTA MEMBUAT/MENULIS APLIKASI WEB, PROYEK KODING, SKRIP PERANGKAT LUNAK, WEBSITE, ATAU PROGRAM LENGKAP:
   - Jawab secara singkat dan antusias bahwa Anda akan membuka CodeDance Autonomous Coding IDE untuk merancang dan mengeksekusi proyek koding tersebut.
   - Sertakan tag eksekusi di akhir respons: [REQUEST_CODEDANCE: Deskripsi Proyek Koding Lengkap]
   - Contoh:
     User: "Buatkan web aplikasi kalkulator React dan Tailwind"
     AI: "Siap! Mari kita bangun aplikasi web kalkulator interaktif tersebut di CodeDance Autonomous IDE dengan preview langsung dan arsitektur kode lengkap.

     [REQUEST_CODEDANCE: Web Aplikasi Kalkulator Modern dengan React dan Tailwind]"
...
// Local memory auto-injection is disabled by default to avoid hallucinations/misunderstandings.
// Memories are accessed purely on-demand via [RECALL_MEMORY: query] when the AI explicitly needs extra info.
const getLocalMemoryContext = () => '';
...
// Helper to sanitize and normalize message history for OpenAI / DeepSeek format
export const sanitizeAndFormatHistory = (conversationHistory = [], currentMessage = '') => {
  ...
  // Take only the last 30 messages to avoid token limit overflow
...
const sendMessageViaBackend = async (message, conversationHistory = [], language = 'id', personality = DEFAULT_PERSONALITY, abortController = null, deepernovaModel = 'deepernova 1.0super flash', userName = '', sessionMessageCount = 0, uploadedImages = [], globalMemory = '', conversationId = null) => {
  ...
  const contextMessages = sanitizeAndFormatHistory(conversationHistory, message);
...
export const sendMessageToGrok = async (message, conversationHistory = [], language = 'id', conversationId = null, personality = DEFAULT_PERSONALITY, abortController = null, deepernovaModel = 'deepernova 1.0super flash', isAuthenticated = false, isGuest = true, userName = '', sessionMessageCount = 0, uploadedImages = []) => {
  ...
  // 🚀 DIRECT INSTANT DISPATCH TO DEEPSEEK (ZERO-DELAY FRONTEND HIT)
  if (!clientThrottle.isAllowed()) {
    throw new Error('Flood protection: Too many requests. Please slow down.');
  }

  const systemPrompt = buildContextualPrompt(conversationHistory, language, message, conversationId, personality, userName, sessionMessageCount, globalMemory) + (systemHistoryText ? `\n\n${systemHistoryText}` : '');
  ...
  for (let idx = 0; idx < TOKENMIX_API_KEYS.length; idx++) {
    ...
    try {
      console.log(`⚡ [INSTANT_AI] Dispatching directly to DeepSeek (${resolvedModel})`);
      const response = await fetchWithTimeout(
        TOKENMIX_API_URL,
        {
          method: 'POST',
          headers: {
            ...
            ...generateClientIntegrityHeaders(),
          },
          signal: abortController?.signal,
          body: JSON.stringify({
      ...
      console.warn(`[INSTANT_AI] Direct DeepSeek attempt failed:`, directErr.message);
      lastError = directErr;
    }
  }

  // Fallback to backend proxy if direct hit was unavailable
  try {
    return await sendMessageViaBackend(message, conversationHistory, language, personality, abortController, effectiveDeepernovaModel, userName, sessionMessageCount, safeUploadedImages, globalMemory, conversationId);
  } catch (backendErr) {
    throw lastError || backendErr;
  }
};
...
export const sendAgenticMessage = async (messages, abortController = null) => {
  ...
  const resolvedModel = 'deepseek-chat';
  
  // Try direct DeepSeek first
  if (apiKey) {
    console.log(`[AGENTIC] Direct DeepSeek call (${resolvedModel}, ${messages.length} messages)`);
