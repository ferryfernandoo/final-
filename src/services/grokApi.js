...
// Deepernova Model Mapping to DeepSeek backend
...
const normalizeDeepernovaModel = (deepernovaModel = 'deepernova 1.0super flash') => {
  ...
  if (trimmed.includes('deepernova')) {
    ...
    if (trimmed.includes('super') || trimmed.includes('1.0') || trimmed.includes('1.2') || trimmed.includes('flash')) return 'deepernova 1.0super flash';
  }

  if (trimmed.includes('super') || trimmed.includes('1.0') || trimmed.includes('flash')) return 'deepernova 1.0super flash';
  if (trimmed.includes('deepseek') || trimmed.includes('gpt') || trimmed.includes('luna') || trimmed.includes('grok')) return trimmed;
...
export const getTokenMixModel = (deepernovaModel = 'deepernova 1.0super flash', hasImages = false) => {
  if (hasImages) return 'deepseek-v4-flash-vision-exp';
  ...
  if (typeof normalizedModel === 'string' && (normalizedModel.includes('deepseek') || normalizedModel.includes('gpt') || normalizedModel.includes('luna') || normalizedModel.includes('grok'))) {
    return normalizedModel.includes('deepseek') ? normalizedModel : 'deepseek-v4-flash-vision-exp';
  }
...
export const sendAgenticMessage = async (messages, abortController = null) => {
  ...
  const resolvedModel = 'deepseek-v4-flash-vision-exp';
