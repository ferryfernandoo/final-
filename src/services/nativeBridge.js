...
class NativeBridge {
  ...
  async createCalendarEvent({ title, datetime, description, location }) {
    ...
    if (this.isNative && cap && cap.Capacitor) {
      try {
        ...
        console.warn('[NativeBridge] Java plugin createCalendarEvent error, trying intent fallback:', e);
      }
      ...
      const intentUri = [
        ...
        `S.description=${encodeURIComponent(description || 'Dibuat via Deepernova AI')}`,
   ...
   * Uses native Java plugin first (startActivity in Java), fallback to intent URI
   */
  async createNativeAlarm({ title, datetime }) {
    ...
    if (this.isNative && cap && cap.Capacitor) {
      try {
        ...
        console.warn('[NativeBridge] Java plugin createAlarm error, trying intent fallback:', e);
      }

      // Android SET_ALARM Intent fallback
      const intentUri = [
        'intent:#Intent',
        'action=android.intent.action.SET_ALARM',
        `i.android.intent.extra.alarm.HOUR=${hour}`,
        `i.android.intent.extra.alarm.MINUTES=${minute}`,
        `S.android.intent.extra.alarm.MESSAGE=${encodeURIComponent(title || 'Alarm Deepernova')}`,
