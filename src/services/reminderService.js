...
class ReminderService {
   ...
   * Uses multiple strategies to ensure it actually opens in the calendar app
   */
  syncToDeviceCalendar(reminder) {
    ...
    if (isIOS) {
      // iOS: data URI with text/calendar MIME type triggers Calendar app
      const dataUri = 'data:text/calendar;charset=utf8,' + encodeURIComponent(icsContent);
      window.open(dataUri, '_self');
    } else if (isAndroid) {
      // Android: Use blob URL with window.open to trigger system calendar intent handler
      // This opens the "Open with" dialog pointing to Calendar apps instead of just downloading
      const blob = new Blob([icsContent], { type: 'text/calendar;charset=utf-8' });
      const blobUrl = window.URL.createObjectURL(blob);
      
      // Strategy 1: Open blob URL in new window — Android will trigger intent chooser for .ics MIME
      const opened = window.open(blobUrl, '_blank');
      
      if (!opened) {
        // Strategy 2: If popup blocked, use data URI approach  
        const dataUri = 'data:text/calendar;charset=utf8,' + encodeURIComponent(icsContent);
      ...
      // Desktop fallback: Download .ics file (desktop OS usually has calendar handler)
  ...
  /**
   * Auto-sync: called automatically when a reminder is created from AI chat.
   * Opens the device calendar directly without requiring user to click a button.
   */
  autoSyncToCalendar(reminder) {
    try {
      // Small delay so the UI card renders first, then trigger calendar open
      setTimeout(() => {
        this.syncToDeviceCalendar(reminder);
      }, 800);
    } catch (e) {
      console.warn('[ReminderService] Auto-sync to calendar failed:', e);
    }
  }
