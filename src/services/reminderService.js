...
class ReminderService {
  constructor() {
    ...
    this.timerId = null;
    this.initTimer();
    nativeBridge.setReminderService(this);
  }
  ...
  addReminder({ title, datetime, type = 'reminder', description = '', repeat = 'none', location = '' }) {
    ...
    // Trigger Native Android Notification silently if on native app (no intent redirects)
    nativeBridge.scheduleNotification({
      id: newReminder.id,
      title: `⏰ ${newReminder.title}`,
      body: `Waktunya: ${reminderDate.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}`,
      datetime: newReminder.datetime
    });

    // If type is alarm, automatically trigger native Android Clock App Alarm
    if (type === 'alarm') {
      try {
        nativeBridge.createNativeAlarm({
          title: newReminder.title,
          datetime: newReminder.datetime
        });
      } catch (alarmErr) {
        console.warn('[ReminderService] Native alarm trigger error:', alarmErr);
      }
    }

    // Ask for notification permission if needed in browser
  ...
  // Delete reminder and cancel scheduled native device alarm/notification
  deleteReminder(id) {
    this.stopAlarmChime();
    this.reminders = this.reminders.filter((r) => r.id !== id);
    this.saveReminders();
    
    // Automatically cancel scheduled notification on Android device
    try {
      nativeBridge.cancelNotification(id);
    } catch (e) {
      console.warn('[ReminderService] Error cancelling device notification on delete:', e);
    }
  }
  ...
  // Snooze reminder by X minutes (default 10 minutes)
  snoozeReminder(id, minutes = 10) {
    const rem = this.reminders.find((r) => r.id === id);
    if (rem) {
      this.stopAlarmChime();
      const newTime = new Date(Date.now() + minutes * 60 * 1000);
      rem.datetime = newTime.toISOString();
      rem.status = 'active';
      this.saveReminders();
      ...
      nativeBridge.scheduleNotification({
        id: rem.id,
        title: `⏰ ${rem.title}`,
        body: `Snoozed to: ${newTime.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}`,
  ...
  // Dismiss reminder and stop audio ringtone
  dismissReminder(id) {
    const rem = this.reminders.find((r) => r.id === id);
    if (rem) {
  ...
  // Play WebAudio alarm chime directly without external audio files
  playAlarmChime() {
    this.playSynthChime();
  }

  // Stop currently looping alarm ringtone
  stopAlarmChime() {
  ...
  // Web Audio Alarm Synthesizer fallback
  playSynthChime() {
    try {
  ...
  syncToDeviceCalendar(reminder) {
    ...
    const handledNatively = nativeBridge.createCalendarEvent(reminder);
  ...
  parseReminderIntent(text) {
    ...
    // Check relative duration (e.g. "5 menit lagi", "10 menit lagi", "1 jam lagi", "30 detik lagi", "in 10 minutes")
    const minuteRelMatch = lower.match(/(\d+)\s*(?:menit|mins?|minutes?)\s*(?:lagi|from now|later)?/);
    const hourRelMatch = lower.match(/(\d+)\s*(?:jam|hours?|hrs?)\s*(?:lagi|from now|later)/);
    const secondRelMatch = lower.match(/(\d+)\s*(?:detik|secs?|seconds?)\s*(?:lagi|from now|later)/);

    let isRelativeTime = false;
    if (minuteRelMatch && (lower.includes('lagi') || lower.includes('in ') || lower.includes('after '))) {
      const mins = parseInt(minuteRelMatch[1], 10);
      targetDate = new Date(now.getTime() + mins * 60 * 1000);
      isRelativeTime = true;
    } else if (hourRelMatch) {
      const hrs = parseInt(hourRelMatch[1], 10);
      targetDate = new Date(now.getTime() + hrs * 60 * 60 * 1000);
      isRelativeTime = true;
    } else if (secondRelMatch) {
      const secs = parseInt(secondRelMatch[1], 10);
      targetDate = new Date(now.getTime() + secs * 1000);
      isRelativeTime = true;
    }
