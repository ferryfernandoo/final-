...
const AIManagerOffice = ({ user, onNavigate, isAuthenticated }) => {
  ...
  const fetchCloudStorageData = async (silent = false) => {
    ...
    // 3. SILENT BACKGROUND MERGE: If server has files, merge them cleanly & persist
    if (serverFiles.length > 0) {
      try {
        ...
        // 1. Put local files into map first
        localFiles.forEach(f => {
          if (f && f.id) fileMap.set(f.id, { ...f });
        });

        // 2. Put server files on top, preserving local dataUrl/thumbnail/text if server has none
        serverFiles.forEach(f => {
          ...
          fileMap.set(f.id, {
            ...existing,
            ...f,
            dataUrl: (f.dataUrl && f.dataUrl !== '[[stored]]') ? f.dataUrl : (existing.dataUrl || null),
            thumbnail: f.thumbnail || existing.thumbnail || null,
            text: f.text || existing.text || null,
            ownerEmail: f.ownerEmail || existing.ownerEmail || currentOwner
          });
        });
        ...
        // CRITICAL: Save merged data to localStorage so it persists across reloads!
        saveLocalFiles(combined);
      } catch (e) {
        console.warn('Error combining server files:', e);
      }
    }
  };
  ...
  useEffect(() => {
    ...
    // Background sync every 30 seconds (silent, non-blocking) — skip when tab is hidden
  ...
  const handleOpenFile = async (file) => {
    ...
    // If file dataUrl & content are not available locally, fetch full file from server with "loading sedang meminta data dari server" notification
    if (!fileDataUrl && !fileContent && file.id && !file.id.startsWith('upload_') && !file.id.startsWith('folder_')) {
      setOpeningFileMessage('loading sedang meminta data dari server...');
      try {
        const controller = typeof AbortController !== 'undefined' ? new AbortController() : null;
        const tid = controller ? setTimeout(() => controller.abort(), 4000) : null;
        const fetchOpts = { credentials: 'include' };
        if (controller) fetchOpts.signal = controller.signal;
        ...
        if (res.ok) {
          ...
          if (data.success && data.file) {
            fileDataUrl = data.file.dataUrl || null;
      ...
      <div className="android-files-app">
        <header className="android-top-appbar">
          <div className="android-title-group">
            <button className="android-back-btn" onClick={() => {
              if (currentFolderId !== null) {
                handleNavBack();
              } else {
                onNavigate?.('universe');
            ...
            <h2>{currentFolderId ? folderPath[folderPath.length - 1]?.name || 'Folder' : 'Cloud Files (1 GB)'}</h2>
          </div>
