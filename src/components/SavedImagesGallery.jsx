...
import { getSavedImages, deleteSavedImage, downloadImageDirectly } from '../services/imageStorageService';
...
const SavedImagesGallery = ({ isOpen, onClose, isAuthenticated, user }) => {
  ...
  // Fetch saved images when panel opens (works for BOTH local mode and logged-in mode)
  useEffect(() => {
  ...
  const fetchSavedImages = async () => {
    ...
    setError(null);
    try {
      const allImages = await getSavedImages(isAuthenticated, user);
      setImages(allImages || []);
    } catch (err) {
  ...
  const handleDownload = async (image) => {
    try {
      const filename = `deepernova-${(image.id || 'img').substring(0, 8)}.png`;
      await downloadImageDirectly(image.imageUrl, filename);
    } catch (err) {
  ...
  const handleDelete = async (imageId) => {
    ...
    try {
      await deleteSavedImage(imageId, isAuthenticated, user);
