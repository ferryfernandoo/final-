...
const ImageGenerator = ({ isOpen, onClose, prompt, userLanguage = 'id', referenceImage = null }) => {
    ...
    try {
      ...
      if (result.success && result.image.url) {
        ...
        saveImageToGallery({
          ...
          type: isEditMode ? 'edited' : 'generated',
          model: isEditMode ? 'qwen-image-edit-max' : 'qwen-image'
        }).catch(e => console.warn('[ImageGenerator] Auto-save gallery warning:', e));
  ...
  const downloadImage = async () => {
    ...
    try {
      const filename = `${isEditMode ? 'edited' : 'generated'}-image-${Date.now()}.png`;
      await downloadImageDirectly(generatedImageUrl, filename);
    } catch (err) {
