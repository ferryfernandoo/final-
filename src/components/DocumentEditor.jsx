  ...
  const triggerKeystrokeAutoSave = useCallback((html) => {
    ...
    _editTimerRef.current = setTimeout(() => {
      ...
      try {
        ...
        localStorage.setItem('deepernova_cloud_files', JSON.stringify(localCloudFiles));
...
Tulis konten untuk Bab "${currentSection}" sekarang:`;
  ...
  const saveArtifact = useCallback((prompt, response, overrideContent, overrideSheets) => {
    ...
    try {
      ...
      const stored = localStorage.getItem('deepernova_cloud_files');
      ...
      const updatedCloudFiles = [cloudFile, ...currentCloudFiles];
      localStorage.setItem('deepernova_cloud_files', JSON.stringify(updatedCloudFiles));
...
Tugas Anda: Tuliskan HANYA kode JavaScript executable murni (TIDAK BOLEH PAKAI FORMAT MARKDOWN / BACKTICKS ATAU PENJELASAN LAIN).
Kode JS Anda memanipulasi variabel 'sheet' (2D array sel: sheet[r][c]).
Setiap sel adalah objek: { value: string|number, format: { bold: boolean, italic: boolean, fontSize: number, fontColor: string, fillColor: string, halign: 'left'|'center'|'right', valign: 'top'|'middle'|'bottom' } }.
Variabel yang tersedia: sheet, formatRupiah, formatUSD, colIdxToLetter.
Aturan Wajib: Kode JS HARUS diakhiri dengan 'return sheet;'.
...
Instruksi Pengguna: ${targetPrompt}`;
      ...
      let cleanCode = responseText ? responseText.replace(/```javascript/gi, '').replace(/```js/gi, '').replace(/```/g, '').trim() : '';
  ...
  const deleteCloudFile = async (id) => {
    ...
    // Clean up local storage & session storage immediately
    try {
      const localCloudStr = localStorage.getItem('deepernova_cloud_files');
      if (localCloudStr) {
        const localCloudFiles = JSON.parse(localCloudStr).filter(f => f.id !== id);
        localStorage.setItem('deepernova_cloud_files', JSON.stringify(localCloudFiles));
      }
    } catch (_e) {}
