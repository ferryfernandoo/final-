...
Instruksi Pengguna: ${targetPrompt}`;
  ...
  const getEditorUserStorageKey = () => {
    ...
    return `deepernova_cloud_vault_${email}`;
  };
  ...
  const fetchCloudFiles = async () => {
    ...
    try {
      localStorage.removeItem(getEditorCompanySharedKey());
    ...
    const cleanLocal = localFiles.filter(f => !dummyIds.includes(f.id));

    if (cleanLocal.length > 0) {
      setCloudFiles(cleanLocal);
    }
  ...
  const createCloudFolder = async (name) => {
    ...
    try {
      await fetch(`${API_BASE_URL}/api/cloud/folders`, {
        method: 'POST',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const saveActiveFileToCloud = async (fileName) => {
    ...
    try {
      await fetch(`${API_BASE_URL}/api/cloud/save`, {
        method: 'POST',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
  ...
  const loadCloudFile = async (file) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/cloud/files/${file.id}`, {
        credentials: 'include'
      });
      if (response.ok) {
        const data = await response.json();
        ...
        <div className="header-left" style={{ display: 'flex', alignItems: 'center', gap: '20px', height: '100%' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <button 
              ...
              onClick={() => {
                const returnView = (typeof sessionStorage !== 'undefined' ? sessionStorage.getItem('editor_return_view') : null) || 'office';
                onNavigate?.(returnView);
              }} 
              title={(typeof sessionStorage !== 'undefined' && sessionStorage.getItem('editor_return_view') === 'chat') ? "Kembali ke Chat" : "Kembali ke Cloud Storage"} 
