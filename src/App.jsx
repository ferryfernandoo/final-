...
function App() {
  ...
  const handleNavigate = (view, fileType) => {
    if (view === 'documents') {
      try {
        // Track where the user came from before entering documents editor
        const originView = currentView === 'office' ? 'office' : (currentView === 'universe' ? 'universe' : 'chat');
        sessionStorage.setItem('editor_return_view', originView);
      } catch (_) {}
    } else if (view === 'office') {
      if (!isAuthenticated || isGuest) {
        setPendingPostLoginRedirect('office');
        setShowLoginView(true);
        return;
      }
      try {
        // Track where the user entered Cloud Storage from
        sessionStorage.setItem('cloud_entry_view', currentView || 'chat');
      } catch (_) {}
    }
