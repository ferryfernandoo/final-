    ...
    const verifyAuth = async () => {
      ...
      try {
        ...
        if (response.status === 401) {
          ...
          // Auto-enable guest session so web is immediately usable without requiring backend server
        ...
        console.warn('[App] Backend server offline or auth check error, enabling standalone Guest Mode:', error?.message);
        const guestSessionStr = safeGetItem('guestSession');
        let guestUser = null;
        if (guestSessionStr) {
          try { guestUser = JSON.parse(guestSessionStr); } catch (_) {}
        }
        if (!guestUser || !guestUser.guest) {
          guestUser = { name: 'Guest', email: 'guest@deepernova.com', guest: true };
        }
        setIsAuthenticated(false);
        setIsGuest(true);
        setUser(guestUser);
